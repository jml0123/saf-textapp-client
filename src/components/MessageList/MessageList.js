...
export default class MessageList extends Component {
  ...
  isToday = (date) => {
    ...
    return (
      moment(d).format("DD") === moment(today).format("DD") &&
  ...
  isTomorrow = (date) => {
    ...
    return (
      moment(d).format("DD") === moment(tomorrow).format("DD") + 1 &&
  ...
  render() {
    ...
    this.context.messages.forEach((message) => {
      const localizedDate = moment.utc(message.scheduled).local().format();
      const dateWithoutTime = localizedDate.split("T")[0];
      return (messageGroup[dateWithoutTime] = [
    ...
    const messages = Object.entries(messageGroup).map((date, i) => {
      ...
      const pendingMessages = date[1].map((message, i) => {
        ...
        const notQueued =
          moment(message.scheduled).utc() > moment().utc()
      ...
      // Can put below in a separate component
      return (
        <React.Fragment key={i}>
          <p className="day-label">{this.convertToString(date[0])}:</p>
          <ul>{pendingMessages}</ul>
        </React.Fragment>
      );
    });
      ...
      <div className="scheduled">
        ...
        <div className="scheduled-content-container">
          {messages.length ? (
            ...
            <p className="indicator-empty">No messages scheduled</p>
          )}
        </div>
      </div>
    );
