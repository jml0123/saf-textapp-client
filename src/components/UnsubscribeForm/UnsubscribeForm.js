...
export default class UnsubscribeForm extends Component {
  ...
  handleDeleteSubscriber = (e) => {
    ...
    fetch(`${config.API_ENDPOINT}/subscribers/unsubscribe`, {
      ...
      .then((data) => {
        const deleteResult =
          data.deleteCount === 1
            ? `Successfully unsubscribed from ${data.deleteCount} curator`
            : data.deleteCount === 0
            ? `No curators to unsubscribe to!`
  ...
  render() {
    const headerMessage = this.state.result
      ? this.state.result
      : "Unsubscribe from all curators";
    return (
      <div className="unsubscribe-container">
        ...
        <form name="unsubscribe-form" onSubmit={this.handleDeleteSubscriber}>
          ...
          <button type="submit" className="action-critical">
            Unsubscribe
          </button>
        </form>
      </div>
